...
apt-get update
apt-get upgrade -y
apt-get install -y build-essential git flex bison bc cmake scons python-dev python-setuptools python3-dev python3-pip python3-setuptools python3-smbus bluetooth libbluetooth-dev rfkill git lighttpd devscripts debhelper tcl8.6-dev autoconf python3-venv libz-dev libssl-dev tcl-dev chrpath libboost-system-dev libboost-program-options-dev libboost-regex-dev libboost-filesystem-dev net-tools tclx8.4 tcllib itcl3 librtlsdr-dev libbladerf-dev libhackrf-dev liblimesuite-dev libusb-1.0-0-dev libncurses5-dev limesuite liblimesuite-dev limesuite-udev soapysdr-module-lms7 pkg-config patchelf
